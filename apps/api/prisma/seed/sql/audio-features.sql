DROP TABLE IF EXISTS stg_echonest;
...
\copy stg_echonest FROM __ECHONEST_CSV__ WITH (FORMAT csv, HEADER true, DELIMITER ',');
...
SET acousticness = EXCLUDED.acousticness,
...
INSERT INTO rank_artist (
  artist_id,
  rank_artist_discovery,
  rank_artist_familiarity,
  rank_artist_hotttnesss
)
SELECT DISTINCT
  ta.artist_id,
...
JOIN _legacy_id_map m
  ON m.old_id = s.track_id
 AND m.table_name = 'track'
JOIN track_artist_main ta
  ON ta.track_id = m.new_uuid
