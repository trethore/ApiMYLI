...
import { Play, Pause, SkipBack, SkipForward, Volume2, Volume1, VolumeX, ListMusic } from "lucide-react"
...
export default function Player() {
  const { currentTrack, isPlaying, togglePlay, volume, setVolume, currentTime, duration, seek, playNext, playPrevious } = usePlayer()
      ...
      <div className="flex items-center justify-between p-4 w-full h-20 sm:h-auto order-2">
          ...
          <div className="flex items-center gap-4 w-1/2 sm:w-1/3">
            ...
            <div className="flex flex-col overflow-hidden">
              ...
              <span className="text-xs sm:text-sm text-muted-foreground truncate">{currentTrack.artist.join(", ")}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-2 w-auto sm:w-1/3 sm:max-w-md">
            <div className="flex items-center gap-2 sm:gap-4">
              <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-foreground hidden sm:inline-flex" onClick={playPrevious}>
                <SkipBack size={20} />
              </Button>
              <Button 
                size="icon" 
              ...
              <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-foreground hidden sm:inline-flex" onClick={playNext}>
                <SkipForward size={20} />
              </Button>
            </div>
          ...
          <div className="hidden sm:flex items-center justify-end gap-2 w-1/3">
            ...
            <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-foreground" onClick={() => setVolume(volume === 0 ? 1 : 0)}>
